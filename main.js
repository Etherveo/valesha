...
import './config.js'
import { makeSessionSaver } from './lib/sessionSafe.js'
...
const getRegionByPhone = (number) => {
  const pn = new PhoneNumber(number)
  return pn.getRegionCode() || 'ID' // fallback ke ID biar aman
}
...
global.prefix = new RegExp('^[' + (opts['prefix'] || '‎xzXZ/!$Oo+?.\\-').replace(/[|\\{}()[\]^Oo$+*?.\-\^]/g, '\\$&') + ']')
...
if (usePairingCode && !conn.authState.creds.registered) {
  ...
  let phoneNumber = ''
  do {
  phoneNumber = await question(chalk.blueBright('Input nomor dengan kode negara (cth: +628xxxx):\n'))
  phoneNumber = phoneNumber.replace(/\D/g, '')

  const region = getRegionByPhone(phoneNumber)

  if (region) break
  console.log(chalk.red('❌ Nomor tidak valid atau tidak terdeteksi. Coba lagi Senpai~'))
} while (true)
...
async function connectionUpdate(update) {
  ...
  if (lastDisconnect && lastDisconnect.error && lastDisconnect.error.output && lastDisconnect.error.output.statusCode !== DisconnectReason.loggedOut) {
    console.log(chalk.red('⏱️ Koneksi terputus, merestart sistem untuk sinkronisasi ulang memori...'));
    global.reloadHandler(true).catch(console.error);
    // process.exit(1); // ✅ AMAN: Panel akan otomatis menghidupkan ulang bot dengan kunci crypto baru
  } 
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.credsUpdate = makeSessionSaver(saveCreds, 'valesha')
...
filesInit().then(_ => {}).catch(console.error)
