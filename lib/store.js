...
/**
 * @param {import('@whiskeysockets/baileys').WASocket | import('@whiskeysockets/baileys').WALegacySocket} // PREV-LEGACY
...
function bind(conn) {
    ...
    // conn.ev.on('contacts.set', updateNameToDb) // PREV-LEGACY
    // PREV-LEGACY
    ...
    // BENAR: Menggunakan messaging-history.set
    conn.ev.on('messaging-history.set', async ({ chats, contacts, isLatest }) => {
        try {
            if (contacts) updateNameToDb(contacts)
            if (chats) {
                for (let { id, name, readOnly } of chats) {
                    id = conn.decodeJid(id)
                    if (!id || id === 'status@broadcast') continue
